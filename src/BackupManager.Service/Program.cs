...
sealed class SchedulerWorker(ILogger<SchedulerWorker> log) : BackgroundService
{
    ...
    private async Task RunDueJobsAsync(CancellationToken token)
    {
        ...
        foreach (var job in config.Jobs.Where(j => j.State == JobState.Enabled && j.Schedule.Kind != "Manual"))
        {
            ...
            config.LastBackupAttempt = now; config.LastBackupError = null; config.LastBackupStatus = "Running";
            try { var result = await _archive.CreateAsync(job, null, token, Unprotect(config.EncryptedMySqlPassword)); var remote = await UploadAsync(config.RemoteFtp, result, job.Name, job.Schedule, token); job.LastRun = result.CompletedAt; job.NextRun = ScheduleCalculator.Next(job.Schedule, result.CompletedAt); config.LastBackup = result.CompletedAt; config.LastBackupStatus = "Completed"; config.History.Add(new ServiceRunRecord(job.Name, result.CompletedAt, "Completed", result.ArchivePath, result.Sha256, remote)); DeleteLocalArchives(result); log.LogInformation("Scheduled backup {Job} completed and uploaded.", job.Name); }
            catch (Exception ex) { job.NextRun = ScheduleCalculator.Next(job.Schedule, now); config.LastBackupStatus = "Failed"; config.LastBackupError = ex.Message; config.History.Add(new ServiceRunRecord(job.Name, now, "Failed", "", "", null, ex.Message)); log.LogError(ex, "Scheduled backup {Job} failed: {Reason}", job.Name, ex.Message); }
        }
    ...
    private static async Task<string?> UploadAsync(ServiceFtpConfig? ftp, BackupResult result, string jobName, Schedule schedule, CancellationToken token)
    {
...
sealed class ServiceConfig { public bool Paused { get; set; } public int RetentionDays { get; set; } = 7; public List<BackupJob> Jobs { get; set; } = []; public List<ServiceRunRecord> History { get; set; } = []; public DateTimeOffset? LastBackup { get; set; } public DateTimeOffset? LastBackupAttempt { get; set; } public string? LastBackupStatus { get; set; } public string? LastBackupError { get; set; } public string? EncryptedMySqlPassword { get; set; } public ServiceFtpConfig? RemoteFtp { get; set; } }
